...
export const Select: React.FC<{
  ...
  return (
    <div ref={ref} className={`relative ${className}`}>
      ...
      {open && (
        <div className="absolute top-full left-0 mt-1 w-full min-w-[140px] bg-[var(--surface-card)] border border-[var(--border-soft)] rounded-xl shadow-lg z-50 py-1 overflow-y-auto" style={{ maxHeight: '240px' }}>
          {options.map(opt => (
            <button key={opt.value} type="button" onClick={() => { onChange(opt.value); setOpen(false); }}
              className={`w-full text-left px-3 py-1.5 text-sm hover:bg-[var(--surface)] flex items-center gap-2 transition-colors ${opt.value === value ? 'text-[var(--primary)]' : 'text-[var(--text-primary)]'}`}>
