...
NOT EXISTS (
  ...
  WHERE cs.commercial_fingerprint=o.commercial_fingerprint AND cs.state='valid' AND ce.state='valid' AND ce.valid_until>CURRENT_TIMESTAMP(3)
) OR NOT EXISTS (
  ...
  WHERE c.credential_pool_id=o.credential_pool_id AND c.status='active' AND ee.state='valid' AND ee.valid_until>CURRENT_TIMESTAMP(3)
) THEN 1 ELSE 0 END),0)
