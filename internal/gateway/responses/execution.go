...
func (p *Pipeline) Create(ctx context.Context, userID, tokenID uint, request *protocol.Request, idempotencyKey string) (*Result, error) {
	if p == nil || p.v2 == nil || p.engine == nil {
		return nil, errors.New("Gateway V2 engine is not initialized")
	}
	return p.createV2(ctx, userID, tokenID, request, idempotencyKey)
}
