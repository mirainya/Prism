...
func TestReadAsyncDispatchIncludesCallbackBindingToken(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT c.id,a.id,a.credential_id")).
		WithArgs(uint64(19)).
		WillReturnRows(sqlmock.NewRows([]string{
			"call_id", "attempt_id", "credential_id", "credential_blob_id", "request_blob_id", "task_identity_blob_id",
			...
			1, 2, 3, 4, 5, 6, 7, 8, 9, "call-1", "json_task_v1", "https://provider.example",
