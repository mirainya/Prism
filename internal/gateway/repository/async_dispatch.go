...
func (s *Store) ReadAsyncDispatch(ctx context.Context, asyncID uint64) (AsyncDispatch, error) {
	...
	err := s.db.QueryRowContext(ctx, `SELECT c.id,a.id,a.credential_id,v.encrypted_blob_id,p.encrypted_blob_id,i.encrypted_blob_id,
...
WHERE x.id=?`, asyncID).Scan(&out.CallID, &out.AttemptID, &out.CredentialID, &out.CredentialBlobID, &out.RequestBlobID, &identity,
