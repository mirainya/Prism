...
AND r.status='ready' AND r.expires_at>CURRENT_TIMESTAMP(3) AND r.adapter_digest=?
...
AND g.status IN ('preparing','active') AND c.status='published' AND r.status='ready' AND r.expires_at>CURRENT_TIMESTAMP(3)
