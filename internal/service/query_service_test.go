...
func TestListAvailableCapabilitiesExcludesUnexecutableCatalogRoutes(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "invalid entitlement", change: `UPDATE gw_credential_entitlement_state SET state='drift'`},
		{name: "expired entitlement evidence", change: `UPDATE gw_credential_validation_events SET valid_until=datetime('now','-1 hour')`},
		{name: "invalid commercial state", change: `UPDATE gw_commercial_state SET state='unknown'`},
		{name: "expired commercial evidence", change: `UPDATE gw_commercial_validation_events SET valid_until=datetime('now','-1 hour')`},
...
func setupPublicCatalogQueryDB(t *testing.T) *sql.DB {
	...
	statements := []string{
		...
		`CREATE TABLE gw_credentials(id INTEGER PRIMARY KEY,channel_id INTEGER,credential_pool_id INTEGER,status TEXT,secret_identity_id INTEGER,current_version_id INTEGER)`,
		...
		`CREATE TABLE encrypted_blobs(id INTEGER PRIMARY KEY,keyring_id INTEGER)`,
		...
		`CREATE TABLE gw_sell_rates(id INTEGER PRIMARY KEY,release_id INTEGER,sku_id INTEGER,rate_evidence_review_event_id INTEGER,component_code TEXT,unit_code TEXT,quantity_source TEXT,charge_event TEXT,unit_price TEXT,unit_scale INTEGER,quantity_step TEXT,max_quantity TEXT,currency_code TEXT,currency_version INTEGER)`,
		...
		`INSERT INTO gw_credentials VALUES (100,50,90,'active',110,120)`,
		...
		`INSERT INTO encrypted_blobs VALUES (130,140)`,
		...
		`INSERT INTO gw_sell_rates VALUES (501,1,30,201,'input','token','usage.input_tokens','call.succeeded','0.000003',0,'0','1000000','USD',1)`,
		`INSERT INTO gw_sell_rates VALUES (502,1,30,201,'output','token','usage.output_tokens','call.succeeded','0.000012',0,'0','1000000','USD',1)`,
		`INSERT INTO gw_sell_rates VALUES (503,1,31,201,'request','request','one','call.succeeded','0.02',0,'0','1','USD',1)`,
	}
