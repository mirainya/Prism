...
// deferred synchronous request is queued. Secrets and payloads remain behind
// encrypted blob references and are opened only by the claiming worker.
...
a.route_id,a.offering_id,a.product_transport_id,ct.id,a.credential_pool_id,a.credential_id,
a.credential_version_id,a.purpose_grant_id,cv.encrypted_blob_id,request_blob.encrypted_blob_id,
...
JOIN gw_credential_versions cv ON cv.id=a.credential_version_id AND cv.credential_id=a.credential_id AND cv.encrypted_blob_id IS NOT NULL
