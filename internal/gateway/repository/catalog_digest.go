...
var catalogDigestQueries = []string{
	...
	`SELECT id,model_operation_id,sku_code,variant_code,delivery_mode,max_results,idempotency_mode,service_tiers FROM gw_skus WHERE release_id=? ORDER BY id`,
	...
	`SELECT r.id,r.sku_id,r.component_code,r.unit_code,r.quantity_source,r.charge_event,r.unit_price,r.pricing_mode,r.pricing_expr,r.max_price,r.unit_scale,r.quantity_step,r.max_quantity,r.currency_code,r.currency_version,e.rate_evidence_id,e.decision FROM gw_sell_rates r JOIN gw_rate_evidence_review_events e ON e.id=r.rate_evidence_review_event_id WHERE r.release_id=? ORDER BY r.id`,
	`SELECT id,offering_id,plan_code FROM gw_cost_plans WHERE release_id=? ORDER BY id`,
	`SELECT r.id,r.cost_plan_id,r.component_code,r.unit_code,r.quantity_source,r.charge_event,r.unit_price,r.pricing_mode,r.pricing_expr,r.max_price,r.unit_scale,r.quantity_step,r.max_quantity,r.currency_code,r.currency_version,e.rate_evidence_id,e.decision FROM gw_cost_rates r JOIN gw_rate_evidence_review_events e ON e.id=r.rate_evidence_review_event_id WHERE r.release_id=? ORDER BY r.id`,
	`SELECT id,sku_id,path FROM gw_sku_downstream_paths WHERE release_id=? ORDER BY id`,
}

func catalogContentDigest(ctx context.Context, tx *sql.Tx, releaseID uint64) (string, error) {
	hasher := sha256.New()
	// v2 adds SKU variants, expression pricing and declared downstream paths.
	// Releases published under v1 keep their stored hash; readiness compares
	// stored values and never recomputes a published release.
	writeDigestValue(hasher, []byte("prism-catalog-content-v2"))
