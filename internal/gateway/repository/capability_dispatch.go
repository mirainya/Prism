...
func (s *Store) ReadCapabilityDispatch(ctx context.Context, attemptID uint64) (CapabilityDispatch, error) {
	...
	err := s.db.QueryRowContext(ctx, `SELECT c.id,a.id,a.credential_id,cv.encrypted_blob_id,p.encrypted_blob_id,
...
WHERE a.id=?`, attemptID).Scan(
		&out.CallID, &out.AttemptID, &out.CredentialID, &out.CredentialBlobID,
