...
import { fetchUnifiedCalls, fetchUnifiedCatalog, fetchUnifiedCredentials, fetchUnifiedGatewayOverview, publishUnifiedCatalog, retireUnifiedCatalog, UnifiedCall, UnifiedCatalogRelease, UnifiedCredential, UnifiedGatewayOverview } from '../services/unifiedGatewayApi';
...
const UnifiedGateway: React.FC = () => {
  ...
  return (
    <div className="space-y-5">
      ...
      </div> : <UnifiedTable tab={tab} catalog={catalog} credentials={credentials} calls={calls} pageInfo={pageInfo} onPage={page => void loadTab(tab, page)} onCatalogAction={changeCatalogState} />}
    </div>
  );
};

type UnifiedTableProps = { tab: 'catalog' | 'credentials' | 'calls'; catalog: UnifiedCatalogRelease[]; credentials: UnifiedCredential[]; calls: UnifiedCall[]; pageInfo: { page: number; pageSize: number; total: number }; onPage: (page: number) => void; onCatalogAction: (release: UnifiedCatalogRelease, action: 'publish' | 'retire') => void };
const UnifiedTable: React.FC<UnifiedTableProps> = ({ tab, catalog, credentials, calls, pageInfo, onPage, onCatalogAction }) => {
  const headers = tab === 'catalog' ? ['版本', '状态', '语义版本', '内容摘要', '发布时间', '操作'] : tab === 'credentials' ? ['凭据', '凭据池', '状态', '版本', '并发限制'] : ['调用 ID', '状态', '报价', '交付方式', '创建时间'];
  const rows = tab === 'catalog' ? catalog.map(item => [<span className="font-bold">#{item.release_no}</span>, <Badge variant={item.status === 'published' ? 'success' : item.status === 'draft' ? 'warning' : 'default'}>{item.status}</Badge>, item.semantic_version, <code className="text-xs">{item.content_hash.slice(0, 12)}…</code>, formatDate(item.published_at || item.created_at), item.status === 'draft' ? <button type="button" onClick={() => onCatalogAction(item, 'publish')} className="font-bold text-[var(--primary)] hover:underline">发布</button> : item.status === 'published' ? <button type="button" onClick={() => onCatalogAction(item, 'retire')} className="font-bold text-amber-600 hover:underline">退役</button> : <span className="text-[var(--text-tertiary)]">—</span>]) : tab === 'credentials' ? credentials.map(item => [<span className="font-bold">{item.credential_code}</span>, item.pool_name || item.pool_code || '—', <Badge variant={item.status === 'active' ? 'success' : item.status === 'draining' ? 'warning' : 'default'}>{item.status}</Badge>, item.current_version_id ? `v${item.current_version_id}` : '未激活', `${item.request_limit ?? '∞'} / ${item.task_limit ?? '∞'}`]) : calls.map(item => [<code className="text-xs">{item.public_id}</code>, <Badge variant={item.status === 'completed' ? 'success' : item.status === 'failed' ? 'error' : 'info'}>{item.status}</Badge>, `${item.quoted_amount} ${item.price_currency}`, item.delivery_mode, formatDate(item.created_at)]);
