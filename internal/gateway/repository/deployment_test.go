...
func TestActivateDeploymentAtomicallyReplacesActiveGeneration(t *testing.T) {
	for _, success := range []bool{true, false} {
		...
		mock.ExpectQuery(`SELECT COUNT\(\*\) FROM crypto_keyring_state`).WillReturnRows(sqlmock.NewRows([]string{"rings"}).AddRow(2))
		mock.ExpectQuery(`SELECT COUNT\(\*\), COALESCE\(SUM`).WithArgs(2).WillReturnRows(sqlmock.NewRows([]string{"members", "ready"}).AddRow(2, 2))
